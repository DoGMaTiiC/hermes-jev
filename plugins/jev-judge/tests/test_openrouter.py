...
def test_auto_backend_priority_matrix():
    cases = (
        ...
        ({"TYPESAFE_API_KEY": None, "AI_GATEWAY_API_KEY": "gw", "OPENROUTER_API_KEY": "or"}, "openrouter"),
        ({"TYPESAFE_API_KEY": None, "AI_GATEWAY_API_KEY": "gw", "OPENROUTER_API_KEY": None}, "gateway"),
    )
    for env, expected in cases:
        with monkeypatch_env(env):
            assert jev.resolve_backend("auto") == expected, (env, expected)
    print("ok  auto backend priority is typesafe -> openrouter -> gateway")
