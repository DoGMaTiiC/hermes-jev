...
default: `TYPESAFE_API_KEY` wins, else `OPENROUTER_API_KEY`, else `AI_GATEWAY_API_KEY`):
...
| `backend`                              | `auto`                                   | `auto` = TypeSafe key, else OpenRouter, else gateway · `typesafe`/`openrouter`/`gateway` forces one |
